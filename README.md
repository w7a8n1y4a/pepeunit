...
- устройства на `esp8266` поидее должны иметь два светодиода, один для подключения к интернету и один для подключения к брокеру

0. `Wifi` реле на основе `esp8266` в обычном механическом и реечном исполнении
1. Ручной апликатор на `esp8266` - свичер состояний, простейшее устройство ввода в `mqtt` брокера
