...
## Фикс работы обновлений репозиториев в github и gitlab
- Общий кеш для публичных репозиториев
- Этот кеш имеет настраиваемый период обновления по дефолту 1 час.
- Автоматическое обновление должно быть можно отключить
- Администратор может принудительно заставить этот кеш обновиться целиком
- Пользователь может принудительно заставить обновиться, тогда обновляется кеш, и уже только потом синхронизируется Repo

Для упрвления этим нужен реестр локальных публичных репозиториев, он же потом будет синхронизироваться между инстансами
- uuid
