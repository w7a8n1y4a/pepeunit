...
`d` | `Read/Write` operations: storing the token for [EMQX](/en/deployment/dependencies/emqx) issued by [Backend](/en/deployment/dependencies/backend) and [DataPipe](/en/deployment/dependencies/datapipe), [User](/en/development-pepeunit/mechanics/roles.html#user) verification in the [Telegram Bot](/en/definitions#telegram-bot), [Grafana](/en/deployment/dependencies/grafana) authorization, state storage for the [Telegram Bot](/en/definitions#telegram-bot), sending configuration change commands for [DataPipe](/en/deployment/dependencies/datapipe)
