...
Will be triggered automatically depending on the [Unit update settings](/en/user/unit/create-unit-pepeunit#auto-update-block) and the [mass update flags for the Repo](/en/user/git-repository/settings-repo.md).
...
1. `Log` — sends an [MQTT command](/en/developer/mqtt/default-mqtt-command#log-sync-log-sync-pepeunit) requesting local logs from the [Unit](/en/definitions#unit). This command deletes all logs stored in [Pepeunit](/en/conception/overview) and expects the [Unit](/en/definitions#unit) to send logs via [MQTT](/en/definitions#mqtt). It is especially useful when the [Unit](/en/definitions#unit) has critical errors or was temporarily disconnected from the network.
1. `Reset` — sends an [MQTT command](/en/developer/mqtt/default-mqtt-command#reset-reset-pepeunit) to restart the device or program. Useful when a [Unit](/en/definitions#unit) needs a forced restart without updating firmware, schema, or [env.json](/en/definitions#env-json).
