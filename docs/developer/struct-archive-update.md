...
Pepeunit умеет генеририровать готовые архивы с обновлениями для [Unit](/definitions#unit), при этом алгоритм генерации разный для компилируемых и интерпритируемых Repo,

## Компилируемые

1. Вычисляется таргет версия
1. Добавляются [env.json](/definitions#env-json) и [schema.json](/definitions#schema-json) на основе таргет версии
1. В файл [env.json](/definitions#env-json) записывается таргет версия - ключ `COMMIT_VERSION`
