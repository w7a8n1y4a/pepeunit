...
:::warning
Размер передаваемого [Unit](/definitions#unit) состояния ограничен количеством символов указанных в переменной окружения `PU_MQTT_MAX_PAYLOAD_SIZE` из [Backend ENV](/deployment/env-variables/backend).
:::
