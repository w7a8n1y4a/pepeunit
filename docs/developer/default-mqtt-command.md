...
:::danger
Алгоритм [Unit](/definitions#unit) различается для компилируемых и интерпритируемых Repo.
:::
...
1. Скачать архив tgz, tar или zip с [env.json](/definitions#env-json) и [schema.json](/definitions#schema-json) при помощи REST. `HTTP_TYPE` и `PEPEUNIT_URL` доступны внутри env.json:
...
1. Скачать новую версию файла [schema.json](/definitions#schema-json) через REST. `HTTP_TYPE` и `PEPEUNIT_URL` доступны внутри env.json:
...
Данная команда отправляет [Unit](/definitions#unit) запрос на обновление env.json, [Unit](/definitions#unit) в ответ должен выполнить опредлённый алгоритм действий, приводящий к изменению текущей версии env.json, на новую.
...
1. Скачать новую версию файла [env.json](/definitions#env-json) через REST. `HTTP_TYPE` и `PEPEUNIT_URL` доступны внутри существующего env.json:
