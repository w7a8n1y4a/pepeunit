...
У [Pepeunit](/conception/overview) достаточно строгие требования к размеру репозиториев. Это связано с тем что данные придётся хранить на стороне [Pepeunit](/conception/overview). Поэтому объём стандартно ограничен объёмом в `25МБ`, [Администраторы](/mechanics/roles#admin) узлов могут увеличить данный лимит, но это может привести к задержке некоторых операций.
