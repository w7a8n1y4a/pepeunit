...
Структурые элементы описанные здесь используются Pepeunit для различных целей начиная с генерации прошивок и заканчивая созданием [UnitNode](/definitions#unitnode).
...
1. Cекреты которые не должны быть опубликованы, применимо к Pepeunit - это [env.json](/definitions#env-json) и [schema.json](/definitions#schema-json), в других репозиториях аналогичную функцию часто выполняют `.env` файлы.
