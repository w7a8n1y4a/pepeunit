...
:::warning
Размер передаваемого [Unit](/definitions#unit) сообщения с префиксом `/pepeunit` на конце - ограничен количеством символов указанных в переменной окружения `MQTT_MAX_PAYLOAD_SIZE` из `.env` файла [Backend](/definitions#backend). По умолчанию это значение составляет `50000` символов.
:::
