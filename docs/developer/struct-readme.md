...
Укажите программную платформу на основе которой было создано данное устройстве, здесь [Пользователи](/mechanics/roles.html#user) будут ожидать увидеть язык программирования или фреймворк на основе которого был создан [Unit](/definitions#unit), желательно указывать версию.
...
Чтобы [Пользователи](/mechanics/roles.html#user) могли быстро понять компилируемый ваш [Unit](/definitions#unit) или интерпритируемый добавьте в данном пункте один из двух возможных вариантов:
- Интерпритируемый
- Компилируемый

Данный параметр при создании [Repo](/definitions#repo) в [Pepeunit](/conception/overview) влияет на первую установку и способ доставки программных обновлений
...
Отобразите [env_example.json](/definitions#env-example-json). Это нужно для опытных [Пользователей](/mechanics/roles.html#user), по переменным окружения можно оценить возможности настройки [Unit](/definitions#unit). Подробное описание структуры [env_example.json](/definitions#env-example-json).
...
Опишите какие переменные, отличные от стандартных, вы добавили в [env_example.json](/definitions#env-example-json). Так как переменные напрямую влияют на работу [Unit](/definitions#unit), подробно опишите какая из них за что отвечает.
...
Отобразите [schema_example.json](/definitions#schema-example-json) . Это нужно для опытных [Пользователей](/mechanics/roles.html#user), по данной схеме можно очень быстро оценить что может [Unit](/definitions#unit). Подробное описание структуры [schema_example.json](/definitions#schema-example-json) .
