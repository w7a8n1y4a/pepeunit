...
Когда другие пользователи [Pepeunit](/conception/overview) переходят по ссылке на репозиторий, они первым делом захотят прочитать описание будущего [Unit](/definitions#unit). Опишите в данном пункте функциональные возможности устройства, желательно просто привести возможности по пунктам, чтобы читать было проще.
...
Укажите программную платформу на основе которой было создано данное устройстве, здесь пользователи будут ожидать увидеть язык программирования или фреймворк на основе которого был создан [Unit](/definitions#unit), желательно указывать версию.
...
Чтобы пользователи могли быстро понять компилируемый ваш [Unit](/definitions#unit) или интерпритируемый добавьте в данном пункте один из двух возможных вариантов:
- Интерпритируемый
- Компилируемый

Данный параметр при создании Repo в [Pepeunit](/conception/overview) влияет на первую установку и способ доставки программных обновлений

## Hardware platform

Здесь указывается тип физических устройств на которые пользователи могут ориентироваться при создании физического [Unit](/definitions#unit)
...
Опишите какие физические компаненты потребуются, чтобы воспроизвести [Unit](/definitions#unit). Данный пункт облегчает пользователям поиск номенклатуры материалов для закупок.
...
Пользователи ожидают увидеть в данном пункте схемы работы [Unit](/definitions#unit), это могут быть электрические схемы, схема работы топиков или схема подключения нескольких [Unit](/definitions#unit) к [Pepeunit](/conception/overview).
Видя схему пользователю легче понять как работает ваш [Unit](/definitions#unit) и воспроизвести его.
...
Отобразите env_example.json. Это нужно для опытных пользователей, по переменным окружения можно оценить возможности настройки [Unit](/definitions#unit). Подробное описание структуры env_example.json.
...
Опишите какие переменные, отличные от стандартных, вы добавили в env_example.json. Так как переменные напрямую влияют на работу [Unit](/definitions#unit), подробно опишите какая из них за что отвечает.
...
Расскижите о логике работы вашего [Unit](/definitions#unit), возможно другие пользователи захотят внести вклад и им будет гораздо проще, если они будут знать подробности о том как работают различные режимы вашего [Unit](/definitions#unit).
