...
1. `Auto-update = True` и `Tags-only = False` - [Repo](/definitions#repo) будет каждый час опрашивать внешний репозиторий [Git](/definitions#git) на предмет новых [коммитов](/definitions#git-commit), как только они появились - [коммиты](/definitions#git-commit) локального хранилища Pepunit будут синхронизированы с внешним репозиторием. При этом все дочерние [Unit](/definitions#unit), которые разрешили автоматическое обновление через [Repo](/definitions#repo) - получат запрос на обновление.
