...
:::tip
За счёт политики `LastValue`, данная конфигурация почти не потребляет `CPU` и `RAM`, поскольку постоянно хранится только `1` запись.
```yml
active_period:
  type: Permanent
filters:
...
Например, нам нужно увидеть некоторые ивентовые события, например, открытие заслонки или включение чего-либо. Для этого нужно хранить N записей, и, как только их становится больше определённого числа, затирать старые:
...
:::tip
За счёт дедупликации и `WhiteList`, данная конфигурация почти не потребляет `CPU` и `RAM`. Также сама политика не позволяет создать больше `~= 512` записей.
```yml
active_period:
  type: FromDate
...
:::tip
За счёт разрешения `max_rate`, данная конфигурация почти не потребляет `CPU` и `RAM`. Число записей для данной политики будет постоянным `~= 1440`.
```yml
active_period:
  type: ToDate
...
:::tip
За счёт применения агрегации, данная конфигурация почти не потребяет `CPU` и `RAM`. Данный `DataPipe` будет создвать `96` записей в бд в день.
```yml
active_period:
  type: DateRange
  start: '2023-11-15T00:00:00+00:00'
