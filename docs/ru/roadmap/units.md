...
1. Переменные:
    - FF_RECONNECTION_TYPE - тип переподключения до wifi
    - FF_LOW_ENERGY_TYPE - тип энергосбережения
    - FF_TIMER_COMMAND_ENABLE - разрешены ли команды включения по таймеру
    - PIN_RELAY_PWM_FREQUENCY - частота шим сигнала
    - PIN_RELAY - номер реле пина
...
1. Установка сообщений для каждого типа нажатий через env

### Экран

1. Простой олед 128/64, выводить фреймы с нужной частотой
1. Разное подключение i2c spi
1. Отдельный топик вывода фрейма целиком
1. Отдельный топик вывода текста по координатам
1. Подумать о нужности создания предварительного буфера, для расчёта только того что нужно обновлять
1. Настройки размера и адреса через env
1. Настройки fps
1. Найстроки пинов
1. Настройки частоты spi i2c
1. Топик обнуления кадра
1. Возможность обновлять только определённые кусочки кадра в отдельном топике. Буквально частичное обновление. N юнитов смогут тогда писать на один экран
