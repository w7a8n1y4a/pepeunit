...
1. В будущем, возможно, будет функционал, при включении которого деньги будут идти в том числе и создателям Pepeunit на кошелёк, но траты с этого кошелька будут регламентированными. Подразумевается, что этот функционал будет предоставлять по специальной лицензии, или вообще будет отдельным микросервисом
1. 100% покрытие модульными тестами для кода денег
1. 100% интеграционное покрытие для кода денег

## Данные для монетизации MessageProcessor

DataPipe -> MessageProcessor

1. Сервис подписан на все сообщения с форматом +/+/pepeunit и +/+
1. Сервис считает все сообщения 
1. Сервис включает в себя функционал DataPipe, но при этом расширяет функционал при помощи счётчиков
...
1. Для каждого Unit Вычисляется динамика памяти в MessageProcessor
...
1. Кран для добавления всем или конкретным пользователям денег на счёт
...
## Борд графана
Общедоступный борд монетизации на каждом инстансе, сколько денежек вошло на инстанс и сколько было потрачено, за определённое время
