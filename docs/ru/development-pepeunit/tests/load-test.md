...
`LOCUST_USERS` | Число [Пользователей](/development-pepeunit/mechanics/roles#user), которые будут создавать нагрузку
`LOCUST_SPAWN_RATE` | Нарастание числа [Пользователей](/development-pepeunit/mechanics/roles#user), от `0` до `400`, c шагом `10`, т.е. нарастание нагрузки займёт `~= 40 секунд`
