...
Если состояние флага перезаписываемости `False`, то [Unit](/definitions#unit), у которых есть [доступ](/development-pepeunit/mechanics/permission), не смогут записать новое значение через [REST](/definitions#rest) и [GQL](/definitions#gql). Т.е. возможность установить значение будет только у [Пользователей](/development-pepeunit/mechanics/roles#user) с [доступом](/development-pepeunit/mechanics/permission) и связанных через `Output->Input` топиков.
