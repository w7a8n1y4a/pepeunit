...
:::danger
До нажатия кнопки `Update`, [системные переменные](/developer/files/struct-env-example-json) c префиксом `PU_` будут генерироваться заново каждый раз при запросе переменных. После нажатия `Update` они зафиксируются. 
:::
...
:::info
Для [Micropython](/definitions#micropython) потребуется предварительно установить интерпретатор на физический [Unit](/definitions#unit)
:::
