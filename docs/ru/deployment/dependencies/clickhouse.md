...
:::danger
Стандартный контейнер `Clickhouse` запускается не на всех `arm` системах, т.к. требует инструкции `ARMv8.2-A`. Из-за этого `Pepeunit` нельзя запустить на `ЭВМ` серии `rpi` версии `4` и ниже.
