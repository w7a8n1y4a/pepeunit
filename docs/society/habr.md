...
Всё лето 2024 я наращивал объём бизнес логики и подошёл к моменту, когда пора создать фронтенд. У меня была основная идея: графовое отображение + модалки для крудов. Выбор пал на библиотеку ForceGraph, и предстояло сделать выбор Vue или React. Я выбрал React, только потому, что из коробки React выдавал больше fps в ForceGraph. Позже мне конечно сказали, что можно было использовать ванильную библиотеку и получить такой же fps. Но я во фронтенде понимал не очень много и единственное, что меня спасло это llm. Без них я бы потратил пару лет на написание и изучение фронтенда. Качество получившегося фронтенда, по моему бекендерскому мнению точно на 3+.

А так я уже до нового года 2025 года сшил вместе фронтенд и бекенд обозначив это первой версией - 0.0.0. Пришлось сделать много доработок в бекенд части, особенно по схемамв gql и rest. Я старался делать API похожим на открытый, чтобы если кто-то захочет написать свой клиент или воспользоваться ручками, такая возможность была.
