...
:::warning
Пример заполнения переменных есть в [Backend](/definitions#backend), файл `.env_example`. Обычно нужно заполнить только основные переменные т.к. есть [файл генерации .env файлов для сервисов]()
:::
