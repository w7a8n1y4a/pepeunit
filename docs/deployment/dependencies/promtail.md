...
`Promtail` -  это агент сбора логов с открытым исходным кодом, входящий в стек `Grafana Loki`, который читает логи из файлов, системных журналов `systemd-journal`, `Kubernetes-подов` или `docker containers`, добавляет метки `labels` и отправляет их в [Loki](/deployment/dependencies/loki) для хранения и анализа.
