...
:::info
В [Pepeunit](/conception/overview) `Nginx` выполняет роль `Api Gateway` - сервиса, предоставляющего всем агентам единый интерфейс [API](/definitions#api) для [Frontend](/deployment/dependencies/frontend), [Backend](/deployment/dependencies/backend) и [Grafana](/deployment/dependencies/grafana).
