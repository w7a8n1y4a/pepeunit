...
:::info
В [Pepeunit](/conception/overview) `Nginx` выполняет роль `Api Gateway` - сервиса предоставляющего единый интерфейс [API](/definitions#api) для: [Frontend](/definitions#frontend), [Backend](/definitions#backend), [Grafana](/deployment/dependencies/grafana) - всем агентам.
