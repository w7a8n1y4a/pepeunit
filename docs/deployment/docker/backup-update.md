...
1. Запустите `Pepeunit`, чтобы получить корректную версию
    ...
    docker compose up -d
    ```
2. Запустите создание `backup`, не прерывая работу контейнеров
    ```bash
    sudo ./backup.sh backup
    ```
3. Развернуть версию из `backup`, инстанс при этом изначально должен быть полностью выключен командой `docker compose down`
    ```bash
