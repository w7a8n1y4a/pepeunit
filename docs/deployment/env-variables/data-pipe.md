...
:::warning
Пример заполнения переменных есть в [Backend Data Pipe](/definitions#backend-data-pipe), файл `.env_example`. Обычно нужно заполнить только основные переменные т.к. есть [файл генерации .env файлов для сервисов](/deployment/docker/deploy.html#заполнение-первичного-env-фаила)
:::
