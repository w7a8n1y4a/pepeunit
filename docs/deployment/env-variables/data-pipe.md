...
`PU_DP_REDIS_URL` | `redis://redis:6379/0` | Ссылка для доступа к [Redis](/deployment/dependencies/redis) - используется для обмена конфигурациями [UnitNode](/definitions#unitnode) между [Backend](/definitions#backend) и [DataPipe](/definitions#datapipe)
...
`PU_DP_MQTT_KEEPALIVE` | `60` | Максимальный период в секундах между отправками `ping` от [DataPipe](/definitions#datapipe) до [EMQX MQTT Broker](/definitions#mqtt-broker)
