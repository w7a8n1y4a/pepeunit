...
### Сценарий 1 - Создание
- Создать публичный/приватный репозиторий
- Попытаться создать Repo с существующим url
- Попытаться создать Repo с невалидным url

### Cценарий 2 - Получить все комиты по ветке
- Получить первые 10 коммитов
- Проверить первый коммит
- Проверить выборку коммитов для не существующей ветки

### Сценарий 3 - Получение и установка Кредов
Для закрытых репозиториев gitlab и github

- Устанавливаем
- Пытаемся получить от публичного репозитория
- Пытаемся установить для публичного репозитория
- Установка кредов другим пользователем, не тем кто создавал сам Реестр
- Проверяем что после установки вторым, работает для первого

### Сценарий 4 - Множественная выборка
- Получение записей для конкретного пользователя
- Все фильтры
- Получение всех публичных репозиториев

### Сценарий 5 - Обновление локального репозиторияы
- Физическое удаление репозитория
- Попытка обновить с проверкой статуса
...
- Попытаться создать закрытый Repo с невалидными кредами от Registry

### Cценарий 2 - Модификация 
- Установить всем repo default ветку - ставится ещё на этапе создания
...
### Сценарий 3 - Получение доступных платформ
...
### Сценарий 4 - Установить ветку по умолчанию
...
### Сценарий 5 - Удаление Repo
- Удалить Repo к которому не присоеденены Unit

### Сценарий 6 - Удаление RepositoryRegistry
- Попытка удалить если есть Repo
- Удаление RepositoryRegistry, если нет Repo
...
- Активировать DataPipe и для Grafana

### Cценарий 2 - Установка DataPipe
- Установка корректных YAML
- Установка корректных YAML и для Grafana
...
### Сценарий 1 - Остановка эмулятора Unit
- Останавливается эмулятор

### Сценарий 2 - Создание Dashboard
- Создаются два дешборда, один в будущем будет удалён
- Попытка создания с не валидным именем

### Сценарий 3 - Создание Dashboard Panel
- Создаются 4 типа панелей для 4 типов агрегации
- Попытка создания с не валидным именем
- Создание для будущего удаления

### Сценарий 4 - Import для DataPipe
- Генерируются данные для всех политик кроме LastValue
- Данные загружаются в Pepeunit
- Устанавливается значение для LastValue

### Сценарий 5 - Создание связей UnitNode и Panel
- Перебираются панели, типы политик и UnitNode параллельно
- Создаются связи
- Создаётся связь для будущего удаления

### Сценарий 6 - Синхронизация Dashboard
- Проверка корректности синхронизации

### Сценарий 7 - Получение единичного Dashboard
- Просто получаем Dashboard

### Сценарий 8 - Получение cписка Dashboard
- Получаем Dashboard по всем параметрам

### Сценарий 9 - Получение Dashaboard Panels c дочерними UnitNode
- Получаем по uuid Dashboard

### Сценарий 10 - Удаление связи UnitNode Panel
- Удаление связи

### Сценарий 11 - Удаление Panel
- Удаление Panel

### Сценарий 12 - Удаление Dashboard
- Удаление Dashboard
